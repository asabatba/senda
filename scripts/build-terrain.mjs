...
const DEFAULT_TEXTURE_MAX_EDGE = 4096;
...
function assertResolution(image, label) {
	...
	if (
		Math.abs(resolutionX) !== EXPECTED_RESOLUTION ||
		Math.abs(resolutionY) !== EXPECTED_RESOLUTION
	) {
		throw new Error(
...
async function main() {
	const maxEdge = Number.parseInt(
		process.env.TERRAIN_MAX_EDGE ?? String(DEFAULT_MAX_EDGE),
		10,
	);
	if (!Number.isFinite(maxEdge) || maxEdge < 2) {
		throw new Error(
			`Invalid TERRAIN_MAX_EDGE value "${process.env.TERRAIN_MAX_EDGE}".`,
		);
	}
	const textureMaxEdge = Number.parseInt(
		process.env.TERRAIN_TEXTURE_MAX_EDGE ?? String(DEFAULT_TEXTURE_MAX_EDGE),
		10,
	);
	if (!Number.isFinite(textureMaxEdge) || textureMaxEdge < 2) {
		throw new Error(
			`Invalid TERRAIN_TEXTURE_MAX_EDGE value "${process.env.TERRAIN_TEXTURE_MAX_EDGE}".`,
		);
	}
